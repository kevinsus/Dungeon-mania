...
import dungeonmania.battles.BattleFacadeV2;
...
public class Game {
    ...
    private BattleFacadeV2 battleFacade;
    ...
    public Game(String dungeonName) {
        ...
        this.battleFacade = new BattleFacadeV2();
    }
    ...
    public Game interact(String entityId) throws IllegalArgumentException, InvalidActionException {
        ...
        if (e instanceof ZombieToastSpawner)
            registerOnce(() -> ((ZombieToastSpawner) e).onDestroy(map), PLAYER_MOVEMENT, "playerDestroysSpawner");
    ...
    public boolean hasEnemyGoalBeenReached(int enemyGoal) {
        return battleFacade.hasEnemyGoalBeenReached(enemyGoal);
    }

    public int getNumberOfSpawners() {
        return (int) map.getEntities().stream().filter(s -> s instanceof ZombieToastSpawner).count();
    }
